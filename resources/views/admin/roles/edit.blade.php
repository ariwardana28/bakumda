...
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="bg-white shadow-md rounded-lg p-6">
        <h2 class="text-xl font-bold mb-4 text-gray-800">Edit Role: <span class="text-indigo-600">{{ $role->name }}</span></h2>
        ...
        <form action="{{ route('admin.role.update', $role->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" placeholder="Contoh: admin, editor, user"
                                ...
                                @foreach($task->permissions as $permission)
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="checkbox" name="permissions[]" value="{{ $permission->name }}"
                                            class="rounded text-indigo-600 focus:ring-indigo-500" {{ in_array($permission->name, old('permissions', $rolePermissions)) ? 'checked' : '' }}>
