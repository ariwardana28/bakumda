...
@section('content')
    <div class="px-3 md:px-8 py-4 md:py-6 space-y-4 md:space-y-8 max-w-7xl mx-auto">
        ...
        <div
            class="relative bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 rounded-2xl md:rounded-[2rem] p-4 sm:p-6 md:p-10 overflow-hidden shadow-xl md:shadow-2xl text-white border border-white/10">

            {{-- Efek Ambient Glow dalam Banner --}}
            <div
                class="absolute -right-16 -top-16 w-48 h-48 md:w-64 md:h-64 bg-gradient-to-tr from-cyan-500/30 to-blue-600/30 rounded-full blur-2xl md:blur-3xl pointer-events-none">
            </div>
            <div
                class="absolute -left-16 -bottom-16 w-48 h-48 md:w-64 md:h-64 bg-gradient-to-tr from-purple-600/20 to-indigo-600/20 rounded-full blur-2xl md:blur-3xl pointer-events-none">
            </div>

            <div class="relative z-10 max-w-3xl">
                {{-- Badge Kategori Vibrant Glass --}}
                <div
                    class="inline-flex items-center gap-1.5 md:gap-2 mb-2.5 md:mb-4 px-3 md:px-3.5 py-1 md:py-1.5 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 border border-blue-400/30 rounded-full text-cyan-300 text-[10px] md:text-xs font-extrabold uppercase tracking-wider backdrop-blur-md">
                    <span class="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-cyan-400 animate-ping"></span>
                    Pengembangan Kompetensi & Hukum
                </div>

                <h1
                    class="text-xl sm:text-2xl md:text-4xl font-black tracking-tight mb-2 md:mb-3 leading-snug md:leading-tight">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-1 md:gap-2">
            <div>
                ...
                <p class="text-[11px] md:text-sm text-slate-500 mt-0.5">Pilih pelatihan yang sesuai dengan jadwal dan
                    kebutuhan pengembangan profesional Anda.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 pb-12">
            @forelse($pelatihans as $item)
                <div
                    class="bg-white rounded-xl md:rounded-2xl shadow-sm hover:shadow-xl border border-slate-200/80 overflow-hidden flex flex-col justify-between transition-all duration-300 group">
                    ...
                    <div>
                        ...
                        <div class="p-3.5 md:p-5">
                            <span
                                class="text-[10px] md:text-[11px] font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2 md:px-2.5 py-0.5 md:py-1 rounded-md mb-1.5 md:mb-2 inline-block">Pelatihan</span>

                            <h3
                                class="text-sm md:text-base font-bold text-slate-900 mb-1.5 md:mb-2 line-clamp-1 group-hover:text-blue-600 transition-colors">
                                {{ $item->judul }}
                            </h3>
                            ...
                            <div
                                class="space-y-1.5 md:space-y-2 pt-3 md:pt-4 border-t border-slate-100 text-[11px] md:text-xs text-slate-600">
                                <div class="flex items-center gap-2 md:gap-2.5">
                                    <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-400 shrink-0" fill="none"
                                ...
                                <div class="flex items-center gap-2 md:gap-2.5">
                                    ...
                                    <span class="font-medium text-slate-700">
                                        Biaya: <strong
                                            class="{{ $item->harga == 0 ? 'text-emerald-600' : 'text-slate-900' }}">
                    ...
                    <div class="p-3.5 md:p-5 pt-0">
                        ...
                        @if ($item->status == 'akan datang')
                            <a href="{{ route('user-pelatihan.show', $item) }}"
                                class="flex items-center justify-center w-full bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs md:text-sm py-2.5 md:py-3 px-4 rounded-xl transition-all shadow-sm">
            ...
            @empty
                <div
                    class="col-span-full text-center py-12 md:py-16 bg-white rounded-2xl border border-dashed border-slate-200 shadow-sm">
                    <div
                        class="w-12 h-12 md:w-16 md:h-16 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center mx-auto mb-3 md:mb-4">
                    ...
                    <p class="text-slate-500 text-xs md:text-sm mt-1">Silakan cek kembali secara berkala untuk program
                        pelatihan terbaru.</p>
                </div>
            @endforelse
        </div>
