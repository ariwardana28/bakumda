...
<div class="container mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <h2 class="text-2xl md:text-3xl font-bold text-gray-800 tracking-tight">Edit Soal</h2>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
        <form action="{{ route('admin.materi.soal.update', ['materi' => $materi->id, 'soal' => $soal->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                {{-- Pertanyaan Soal --}}
                ...
                <div class="space-y-4 pt-2 border-t border-gray-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Perbarui teks jawaban dan ubah tanda centang radio untuk kunci jawaban yang benar.</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-3 pt-2">
                        @foreach ($soal->jawaban as $key => $jawaban)
                            @php
                                // Menentukan status checked berdasarkan old input atau status asli dari database
                                $isChecked = old('jawaban_benar', $soal->jawaban->firstWhere('status', 1)?->id) == $jawaban->id;
                            @endphp
                            <div class="flex items-center gap-3 p-3 bg-gray-50/70 border border-gray-200 rounded-xl hover:border-indigo-300 transition duration-200">
                                <div class="flex items-center h-5">
                                    {{-- Radio button bernilai ID jawaban --}}
                                    <input 
                                        type="radio" 
                                        id="jawaban_benar_{{ $jawaban->id }}" 
                                        name="jawaban_benar" 
                                        value="{{ $jawaban->id }}" 
                                ...
                                <label for="jawaban_benar_{{ $jawaban->id }}" class="flex-shrink-0 w-6 h-6 rounded-lg bg-white border border-gray-200 flex items-center justify-center font-bold text-xs text-gray-700 shadow-xs cursor-pointer">
                                    {{ chr(65 + $key) }}
                                </label>
                                
                                {{-- Hidden input untuk membawa ID jawaban --}}
                                <input type="hidden" name="jawaban[{{ $key }}][id]" value="{{ $jawaban->id }}">
                                
                                {{-- Input teks jawaban --}}
                                <input 
                                    type="text" 
                                    name="jawaban[{{ $key }}][text]" 
                                    class="w-full bg-transparent border-0 text-sm text-gray-800 focus:ring-0 focus:outline-none placeholder-gray-400" 
                                    placeholder="Tuliskan pilihan jawaban {{ chr(65 + $key) }}..." 
                                    value="{{ old('jawaban.'.$key.'.text', $jawaban->jawaban) }}" 
                                    required>
                            </div>
                        @endforeach
                    </div>
