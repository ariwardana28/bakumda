            ...
            <form id="registrationForm" action="{{ route('user-anggota.store') }}" method="POST"
                ...
                <div>
                    <div class="flex items-center gap-2 mb-3 md:mb-4 pb-2 border-b border-slate-100 dark:border-slate-800">
                        <div
                            class="w-6 h-6 md:w-7 md:h-7 rounded-lg bg-orange-50 text-orange-600 dark:bg-orange-500/10 dark:text-orange-400 flex items-center justify-center text-xs font-bold">
                            1
                        </div>
                        <h3
                            class="text-xs md:text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 md:gap-4">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', Auth::user()->name ?? '') }}"
                                required placeholder="Contoh: Nama Lengkap, S.H., M.H."
                        ...
                        <div>
                            ...
                            @php $jenisKelaminValue = old('jenis_kelamin', Auth::user()->jenis_kelamin ?? ''); @endphp
                        ...
                        <div>
                            ...
                            @php
                                ...
                                if (!$tanggalLahirValue && Auth::user()?->tanggal_lahir) {
                                    $tanggalLahirValue = \Illuminate\Support\Carbon::parse(
                        ...
                        <div>
                            ...
                            @php $agamaValue = old('agama', Auth::user()->agama ?? ''); @endphp
                            <select name="agama" required
                                class="w-full px-3.5 md:px-4 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800/60 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-800 focus:border-orange-500/50 focus:ring-2 focus:ring-orange-500/10 outline-none transition-all @error('agama') border-rose-500 @enderror">
                                <option value="">-- Pilih Agama --</option>
                                <option value="Islam" {{ $agamaValue == 'Islam' ? 'selected' : '' }}>Islam</option>
                                <option value="Kristen Protestan"
                                    {{ $agamaValue == 'Kristen Protestan' ? 'selected' : '' }}>Kristen Protestan</option>
                                ...
                                <option value="Khonghucu" {{ $agamaValue == 'Khonghucu' ? 'selected' : '' }}>Khonghucu
                                </option>
                            </select>
                            @error('agama')
                                <span class="text-[10px] text-rose-500 mt-1 block">{{ $message }}</span>
                        ...
                        <div>
                            ...
                            @php $statusPerkawinanValue = old('status_perkawinan', Auth::user()->status_perkawinan ?? ''); @endphp
                            <select name="status_perkawinan" required
                                ...
                                <option value="Belum Kawin"
                                    {{ $statusPerkawinanValue == 'Belum Kawin' ? 'selected' : '' }}>Belum Kawin</option>
                                <option value="Kawin" {{ $statusPerkawinanValue == 'Kawin' ? 'selected' : '' }}>Kawin
                                </option>
                                <option value="Cerai Hidup"
                                    {{ $statusPerkawinanValue == 'Cerai Hidup' ? 'selected' : '' }}>Cerai Hidup</option>
                                <option value="Cerai Mati" {{ $statusPerkawinanValue == 'Cerai Mati' ? 'selected' : '' }}>
                                    Cerai Mati</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="kewarganegaraan" required
                                ...
                                <option value="WNI"
                                    {{ old('kewarganegaraan', Auth::user()->kewarganegaraan ?? 'WNI') == 'WNI' ? 'selected' : '' }}>
                                    WNI
                ...
                <div>
                    <div class="flex items-center gap-2 mb-3 md:mb-4 pb-2 border-b border-slate-100 dark:border-slate-800">
                        <div
                            class="w-6 h-6 md:w-7 md:h-7 rounded-lg bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 flex items-center justify-center text-xs font-bold">
                            2
                        </div>
                        <h3
                            class="text-xs md:text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 md:gap-4">
                        ...
                        <div>
                            ...
                            @php $provinsiValue = old('provinsi', Auth::user()->provinsi ?? ''); @endphp
                            <div class="relative">
                                <span
                                    class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
                                    <i class="fa-solid fa-magnifying-glass text-xs"></i>
                                </span>
                                <input type="text" id="provinsiSearch" placeholder="Cari atau pilih Provinsi..."
                                    value="{{ $provinsiValue }}" autocomplete="off"
                                ...
                                <input type="hidden" name="provinsi" id="provinsiValue" value="{{ $provinsiValue }}">

                                <div id="provinsiDropdown"
                                    class="absolute z-30 left-0 right-0 mt-1 max-h-56 overflow-y-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg hidden">
                        ...
                        <div>
                            ...
                            @php $kotaValue = old('kota', Auth::user()->kota ?? ''); @endphp
                            <div class="relative">
                                <span
                                    class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
                                    <i class="fa-solid fa-magnifying-glass text-xs"></i>
                                </span>
                                <input type="text" id="kotaSearch"
                                    placeholder="{{ $kotaValue ? '' : 'Pilih provinsi terlebih dahulu...' }}"
                                    value="{{ $kotaValue }}" autocomplete="off" {{ $kotaValue ? '' : 'disabled' }}
                                ...
                                <input type="hidden" name="kota" id="kotaValue" value="{{ $kotaValue }}">
                        ...
                        <div>
                            ...
                            @php $kecamatanValue = old('kecamatan', Auth::user()->kecamatan ?? ''); @endphp
                            <div class="relative">
                                <span
                                    class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
                                    <i class="fa-solid fa-magnifying-glass text-xs"></i>
                                </span>
                                <input type="text" id="kecamatanSearch"
                                    placeholder="{{ $kecamatanValue ? '' : 'Pilih kota/kabupaten terlebih dahulu...' }}"
                                    value="{{ $kecamatanValue }}" autocomplete="off"
                                    {{ $kecamatanValue ? '' : 'disabled' }}
                        ...
                        <div>
                            ...
                            @php $kelurahanValue = old('kelurahan', Auth::user()->kelurahan ?? ''); @endphp
                            <div class="relative">
                                <span
                                    class="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
                                    <i class="fa-solid fa-magnifying-glass text-xs"></i>
                                </span>
                                <input type="text" id="kelurahanSearch"
                                    placeholder="{{ $kelurahanValue ? '' : 'Pilih kecamatan terlebih dahulu...' }}"
                                    value="{{ $kelurahanValue }}" autocomplete="off"
                                    {{ $kelurahanValue ? '' : 'disabled' }}
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="alamat" rows="2" required placeholder="Detail Jalan, No. Rumah, RT/RW..."
                                class="w-full px-3.5 md:px-4 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800/60 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-800 focus:border-orange-500/50 focus:ring-2 focus:ring-orange-500/10 outline-none transition-all @error('alamat') border-rose-500 @enderror">{{ old('alamat', Auth::user()->alamat ?? '') }}</textarea>
                            @error('alamat')
                                <span class="text-[10px] text-rose-500 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="no_ktp" value="{{ old('no_ktp', '') }}" maxlength="16"
                                required placeholder="Masukkan 16 digit nomor KTP"
