        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-4 sticky top-4 self-start space-y-4 z-30">
                <div
                    ...
                    <div class="text-left">
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-600 text-[10px] font-bold uppercase tracking-wider mb-2 border border-blue-100/60 shadow-sm">
                    ...
                    <div id="timer-container"
                        class="flex items-center gap-3 px-4 py-3 bg-blue-50/80 backdrop-blur border border-blue-200/60 rounded-xl text-blue-700 font-extrabold shadow-sm transition-all duration-300 w-full justify-between">
                        <div class="flex items-center gap-2.5">
                            <div
                                class="w-7 h-7 rounded-lg bg-blue-600 text-white flex items-center justify-center shadow-md shadow-blue-500/20 shrink-0">
                            ...
                            <div class="flex flex-col text-left">
                                <span class="text-[9px] text-slate-400 uppercase tracking-widest font-bold">Sisa
                                    Waktu</span>
                    ...
                    <button type="button" id="btn-fullscreen" onclick="toggleFullscreen()"
                        class="w-full px-3 py-2 bg-slate-900 text-white text-[11px] font-bold rounded-lg hover:bg-blue-600 transition shadow-sm hidden">
                        Fullscreen-kan Layar
                    </button>

                    <div class="text-[11px] text-slate-400 font-medium leading-relaxed border-t border-slate-100 pt-3">
                        * Dilarang keluar dari mode fullscreen, beralih tab browser, atau me-refresh halaman selama ujian
                        berlangsung.
                    </div>
                </div>
            </div>
                ...
                <form id="quiz-form" action="{{ route('user-soal.store', [$pelatihan->id, $materi->id]) }}" method="POST">
                    ...
                    <div class="space-y-4">
                        @forelse($materi->soal as $index => $item)
                            <div
                                class="bg-white rounded-2xl p-4 sm:p-6 shadow-md shadow-slate-100/70 border border-slate-100 transition-all duration-300 hover:shadow-lg group">

                                {{-- Pertanyaan --}}
                                <div class="flex items-start gap-3 mb-4">
                                    <span
                                        class="flex-shrink-0 w-7 h-7 flex items-center justify-center bg-gradient-to-tr from-slate-100 to-slate-200 text-slate-700 rounded-xl font-black text-xs shadow-sm group-hover:from-blue-600 group-hover:to-indigo-600 group-hover:text-white transition-all duration-300">
                                        {{ $index + 1 }}
                                    </span>
                                    <div class="text-slate-900 font-bold text-sm sm:text-base leading-relaxed pt-0.5">
                                        {{ $item->soal }}
                                    </div>
                                </div>

                                {{-- Pilihan Opsi Jawaban (Tanpa Huruf A, B, C, D, E) --}}
                                <div class="space-y-2 pl-0 sm:pl-10">
                                    ...
                                    @foreach ($item->jawaban as $jawabanIndex => $jawaban)
                                        @if (isset($opsiKeys[$jawabanIndex]))
                                            <label
                                                class="relative flex items-center gap-3 p-3 rounded-xl border border-slate-200/80 bg-slate-50/40 hover:bg-blue-50/40 hover:border-blue-400 cursor-pointer transition-all duration-200 shadow-sm">
                                                <input type="radio" name="jawaban[{{ $item->id }}]"
                                                    value="{{ $jawaban->id }}" class="peer sr-only">

                                                {{-- Custom Radio Circle Indicator --}}
                                                <div
                                                    class="w-4 h-4 rounded-full border-2 border-slate-300 peer-checked:border-blue-600 peer-checked:bg-blue-600 flex items-center justify-center transition-all duration-200 shrink-0 shadow-sm">
                                                    <div
                                                        class="w-1.5 h-1.5 rounded-full bg-white scale-0 peer-checked:scale-100 transition-transform duration-200">
                                                    </div>
                                                </div>

                                                {{-- Teks Jawaban (Tanpa Label Abjad) --}}
                                                <span
                                                ...
                                                <div
                                                    class="absolute inset-0 rounded-xl border-2 border-blue-600 opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity duration-200 bg-blue-50/10">
                                                </div>
                                            </label>
                                        @endif
                                    @endforeach
                        ...
                        @empty
                            <div
                                ...
                                <p class="text-slate-500 font-medium text-sm">Belum ada soal yang tersedia untuk materi ini.
                                </p>
                            </div>
                        @endforelse
                        ...
                        @if (isset($materi->soal) && $materi->soal->count() > 0)
                            <div
                                class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 bg-white/10 backdrop-blur p-4 rounded-2xl border border-white/10">
                                <div class="text-[11px] text-white/80 font-medium text-center sm:text-left">
                                    Pastikan semua pertanyaan telah dijawab sebelum mengirimkan.
                                </div>
                                <button type="submit" onclick="isSubmitted = true;"
                                    class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-blue-600/30 transition-all duration-200 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 group">
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                document.addEventListener("fullscreenchange", function() {
                    if (!document.fullscreenElement && !isSubmitted) {
                        ...
                        alert(
                            "PERHATIAN: Anda keluar dari mode Fullscreen! Harap kembali ke mode fullscreen untuk melanjutkan ujian."
                            );
                    } else {
                ...
                document.addEventListener("visibilitychange", function() {
                    if (document.hidden && !isSubmitted) {
                        ...
                        if (warningCount <= maxWarnings) {
                            alert(
                                `PERINGATAN (${warningCount}/${maxWarnings}): Anda terdeteksi mencoba membuka tab/aplikasi lain! Ujian ini harus dikerjakan tanpa beralih halaman.`
                                );
                            toggleFullscreen();
                        } else {
                            alert(
                                "Anda telah melanggar aturan ujian berkali-kali. Ujian akan dikirimkan secara otomatis."
                                );
                            isSubmitted = true;
                ...
                const durationInSeconds =
                    300; // 5 Menit (bisa disesuaikan dari backend misal: {{ $materi->durasi * 60 }})
