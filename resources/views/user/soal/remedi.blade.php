        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-4 sticky top-6 self-start space-y-6 z-30">
                <div class="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl shadow-slate-100 border border-slate-100 flex flex-col gap-6 transition-all">
                    ...
                    {{-- Judul & Status Materi --}}
                    <div class="text-left">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-600 text-xs font-bold uppercase tracking-wider mb-3 border border-amber-100/60 shadow-sm">
                        ...
                        <p class="text-xs text-slate-500 mt-2 font-medium">
                            Nilai Sebelumnya: <span class="font-bold text-slate-700">{{ number_format($nilai->nilai_total_soal, 2) }}</span> (Minimal Kelulusan: 75)
                        </p>
                    </div>

                    {{-- Kotak Timer Mundur --}}
                    <div id="timer-container" class="flex items-center gap-3 px-5 py-4 bg-amber-50/80 backdrop-blur border border-amber-200/60 rounded-2xl text-amber-700 font-extrabold shadow-sm transition-all duration-300 w-full justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-xl bg-amber-600 text-white flex items-center justify-center shadow-md shadow-amber-500/20 shrink-0">
                                <svg class="w-5 h-5 animate-spin-slow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                            </div>
                            <div class="flex flex-col text-left">
                                <span class="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Sisa Waktu Remidi</span>
                    ...
                    <button type="button" id="btn-fullscreen" onclick="toggleFullscreen()" class="w-full px-4 py-3 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-amber-600 transition shadow-sm hidden">
                        Fullscreen-kan Layar
                    </button>

                    <div class="text-xs text-slate-400 font-medium leading-relaxed border-t border-slate-100 pt-4">
                        * Perhatian: Nilai remidi hanya akan memperbarui skor Anda jika hasilnya lebih tinggi atau sama dengan nilai sebelumnya. Dilarang keluar dari mode fullscreen atau beralih tab.
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-8">
                <form id="quiz-form" action="{{ route('user-soal.store-remedi', [$pelatihan->id, $materi->id]) }}" method="POST">
                    @csrf
                    ...
                    <div class="space-y-6">
                        @forelse($materi->soal as $index => $item)
                            <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-100/70 border border-slate-100 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/50 group">
                                ...
                                <div class="flex items-start gap-4 mb-6">
                                    <span class="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-gradient-to-tr from-slate-100 to-slate-200 text-slate-700 rounded-2xl font-black text-sm shadow-sm group-hover:from-amber-600 group-hover:to-orange-600 group-hover:text-white transition-all duration-300">
                                        {{ $index + 1 }}
                                    </span>
                                ...
                                <div class="space-y-3 pl-0 sm:pl-14">
                                    ...
                                    @foreach($item->jawaban as $jawabanIndex => $jawaban)
                                        @if(isset($opsiKeys[$jawabanIndex]))
                                            <label class="relative flex items-center gap-4 p-4 rounded-2xl border border-slate-200/80 bg-slate-50/40 hover:bg-amber-50/40 hover:border-amber-400 cursor-pointer transition-all duration-200 shadow-sm">
                                                <input type="radio" 
                                                ...
                                                <div class="w-5 h-5 rounded-full border-2 border-slate-300 peer-checked:border-amber-600 peer-checked:bg-amber-600 flex items-center justify-center transition-all duration-200 shrink-0 shadow-sm">
                                                ...
                                                <span class="text-slate-700 font-medium text-sm sm:text-base select-none flex-1">
                                                    <strong class="uppercase mr-2 px-2 py-0.5 rounded-lg bg-slate-200/60 text-slate-700 font-bold text-xs peer-checked:bg-amber-100 peer-checked:text-amber-700">{{ $opsiKeys[$jawabanIndex] }}</strong> 
                                                    {{ $jawaban->jawaban }}
                                                </span>

                                                {{-- Active Overlay Highlight for Label --}}
                                                <div class="absolute inset-0 rounded-2xl border-2 border-amber-600 opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity duration-200 bg-amber-50/10"></div>
                                            </label>
                                        @endif
                                    @endforeach
                                </div>
                        ...
                        @empty
                            <div class="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm">
                                ...
                                <p class="text-slate-500 font-medium text-base">Belum ada soal remidi yang tersedia untuk materi ini.</p>
                            </div>
                        @endforelse
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (perfEntries.length > 0 && perfEntries[0].type === "reload") {
                alert("PERHATIAN: Me-refresh halaman selama ujian remidi adalah pelanggaran! Soal telah diacak ulang.");
            }
            ...
            document.addEventListener("fullscreenchange", function () {
                if (!document.fullscreenElement && !isSubmitted) {
                    ...
                    alert("PERHATIAN: Anda keluar dari mode Fullscreen! Harap kembali ke mode fullscreen untuk melanjutkan remidi.");
                } else {
            ...
            document.addEventListener("visibilitychange", function () {
                if (document.hidden && !isSubmitted) {
                    ...
                    if (warningCount <= maxWarnings) {
                        alert(`PERINGATAN (${warningCount}/${maxWarnings}): Anda terdeteksi mencoba membuka tab/aplikasi lain! Ujian remidi ini harus dikerjakan tanpa beralih halaman.`);
                        toggleFullscreen();
                    } else {
                        alert("Anda telah melanggar aturan ujian berkali-kali. Remidi akan dikirimkan secara otomatis.");
                        isSubmitted = true;
            ...
            const timer = setInterval(function () {
                ...
                if (totalTime <= 60) {
                    timerContainer.classList.remove("bg-amber-50/80", "border-amber-200/60", "text-amber-700");
