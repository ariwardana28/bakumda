...
class SoalController extends Controller
{
    ...
    public function store(Request $request, Materi $materi)
    {
        $validated = $request->validate([
            ...
            'jawaban_benar' => 'required|in:0,1,2,3,4',
        ]);
        ...
        try {
            ...
            foreach ($validated['jawaban'] as $index => $jawabanText) {
                // Buat jawaban baru yang langsung terhubung dengan soal
                $soal->jawaban()->create([ // Ini sekarang akan otomatis mengisi soal_id
                    'jawaban' => $jawabanText,
    ...
    public function update(Request $request, Materi $materi, Soal $soal)
    {
        $validated = $request->validate([
            'soal'          => 'required|string',
            'jawaban'       => 'required|array|min:5|max:5',
            'jawaban.*.id'  => 'required|integer|exists:jawaban,id',
            'jawaban.*.text' => 'required|string',
            'jawaban_benar' => 'required|integer|exists:jawaban,id',
        ]);
        ...
        try {
            ...
            // Array untuk menampung ID jawaban yang akan disinkronkan ke tabel pivot 'soal_jawaban'
            $jawabanIds = [];

            // 2. Loop untuk update data di tabel 'jawaban'
            foreach ($validated['jawaban'] as $jawabanData) {
                $jawabanId = $jawabanData['id'];
                $isCorrect = ((int)$jawabanId === (int)$validated['jawaban_benar']);

                // Update teks dan status di tabel 'jawaban'
                ...
                if ($jawaban) {
                    ...
                    // Masukkan ID ke dalam array untuk tabel pivot
                    $jawabanIds[] = $jawabanId;
                }
            }

            // 3. Sinkronisasi relasi ke tabel pivot 'soal_jawaban' 
            // (Memastikan relasi antara soal_id dan jawaban_id tercatat dengan benar di tabel soal_jawaban)
            if (!empty($jawabanIds)) {
                $soal->jawaban()->sync($jawabanIds);
            }
